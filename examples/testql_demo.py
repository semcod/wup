...
def _run_with_mock_services(mapper, mock_services: dict) -> None:
    """Run the demo display using pre-populated mapper and mock_services dict."""
...
def _build_mock_services(mapper) -> dict:
    """Populate mapper with mock TestQL services and return the mock data."""
    mock_services = {
        "testql/commands": {
            "endpoints": [
                "/api/v1/endpoints",
        ...
        "testql/discovery": {
            "endpoints": ["/api/v1/discover", "/api/v1/topology", "/api/v1/inspect"],
            "files": [
        ...
        "testql/adapters": {
            "endpoints": ["/api/v1/adapter/yaml", "/api/v1/adapter/json"],
            "files": [
...
def simulate_testql_analysis(testql_path: str):
    ...
    mock_services = _build_mock_services(mapper)
...
def simulate_with_mock_data():
    """Run demo with completely mock data (no real project path needed)."""
    ...
    print("=" * 60)
    print()
    mapper = DependencyMapper("/mock/testql/path")
    mock_services = _build_mock_services(mapper)
    _run_with_mock_services(mapper, mock_services)
    print("=" * 60)
    print("✅ Demo complete!")
