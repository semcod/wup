...
def create_slack_payload(event: dict) -> dict:
    ...
    if "file" in event:
        fields.append({"title": "File", "value": event["file"], "short": False})

    if "endpoint" in event:
        fields.append({"title": "Endpoint", "value": event["endpoint"], "short": False})

    if "reason" in event:
        fields.append({"title": "Reason", "value": event["reason"], "short": False})
...
def create_teams_payload(event: dict) -> dict:
    ...
    if "from" in event and "to" in event:
        facts.append(
            {"name": "Health Transition:", "value": f"{event['from']} → {event['to']}"}
        )
...
def create_discord_payload(event: dict) -> dict:
    ...
    if "file" in event:
        fields.append({"name": "File", "value": event["file"], "inline": False})
...
class NotificationRouter:
    ...
    def add_slack(self, webhook_url: str):
        ...
        self.channels["slack"] = {"url": webhook_url, "formatter": create_slack_payload}

    def add_teams(self, webhook_url: str):
        """Add Microsoft Teams webhook endpoint."""
        self.channels["teams"] = {"url": webhook_url, "formatter": create_teams_payload}
...
def show_webhook_demo():
    ...
    events = [
        ...
        },
        {
            ...
            "diff": {"added": 15, "removed": 3, "changed": 7},
            "timestamp": int(datetime.now().timestamp()),
        },
    ]
...
def main():
    ...
    parser.add_argument(
        "--test-slack", action="store_true", help="Send test notification to Slack"
    )
    parser.add_argument(
