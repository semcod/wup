...
class CustomTestQLWatcher(WupWatcher):
    ...
    def __init__(
        self,
        testql_project_root: str,
        visual_diff_cfg: Optional[VisualDiffConfig] = None,
        **kwargs,
    ):
        ...
        self.visual_differ = VisualDiffer(
            testql_project_root, visual_diff_cfg or VisualDiffConfig()
        )
    ...
    async def run_quick_test(self, service: str, endpoints: List[str]) -> bool:
        ...
        try:
            ...
            self.console.print(
                f"[green]✓ Quick test passed ({len(test_scenarios)} scenarios)[/green]"
            )
    ...
    async def run_detail_test(self, service: str, endpoints: List[str]) -> Dict:
        ...
        try:
            ...
            if results["failed"] > 0:
                results["blame"] = self._generate_blame_report(
                    service, results["failed_scenarios"]
                )
                self.console.print(
                    f"[red]✗ Found {results['failed']} regression(s)[/red]"
                )
            else:
                self.console.print(
                    f"[green]✓ All {results['passed']} scenarios passed[/green]"
                )
    ...
    def _generate_blame_report(self, service: str, failed_scenarios: List[str]) -> Dict:
        ...
        blame = {"service": service, "files": [], "recent_commits": []}
        ...
        try:
            ...
            result = subprocess.run(
                cmd, cwd=str(self.testql_root), capture_output=True, text=True
            )
...
def main():
    ...
    print("🚀 Starting WUP + TestQL + Visual Diff integration")
    ...
    if deps["services"]:
        ...
        for service, info in sorted(deps["services"].items()):
            print(
                f"  • {service}: {len(info['endpoints'])} endpoints, {len(info['files'])} files"
            )
        print()
